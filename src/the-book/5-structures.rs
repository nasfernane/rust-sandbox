struct User {
    active: bool,
    username: String, // on n'utilise pas de référence ici pour être sûr que la donnée est valide 
    email: String, // tant que la structure est valide
    sign_in_count: u64,
}
...
fn main() {
    ...
    // ces structures sont pratiques quand on a besoin d'implémenter un trait sur un type qui n'a pas de données
    // on peut plus tard implémenter un comportement pour ce type 
