fn main() {
    ...
    // types composés (COMPOUND TYPES)

    // les tuples
    // let tup: (i32, f64, u8) = (500, 6.4, 1); // plusieurs types possibles
    // on peut déstructurer les valzurs
    // let (x, y, z) = tup;
    // println!("The value of y is {y}");
    // println!("The value of z is {}", z);

    // accéder à l'index d'un tuple
    // let five_hundred = tup.0;
    // println!("{five_hundred}");

    // les tableaux - comme tuple mais toutes les valeurs ont le même type
    // ils sont utiles quand la longueur est fixe (sinon utiliser les vecteurs);
    // let a = [1, 2, 3, 4];

    // let two = a[1];
    // println!("{two}");

    // annotation de type = [type; longueur]
    // let a: [i32; 5] = [1, 2, 3, 4, 5];

    // répétition de valeurs => [valeur; longueur]
    let array = [3; 5]; // b vaut [3, 3, 3, 3, 3];
    println!("third value of array: {}", array[3]);

    // accès comme en js
    let _first_value = array[0];
    let _second_value = array[1];
}
